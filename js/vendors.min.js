...
let burgerMenu = document.querySelector('.header__links')
...
// let titlee = document.querySelector(".standart__title");

// let offsetGap = 20; // расстояние между панелями
// let titleOffset = titlee.offsetHeight + 0; // отступ под заголовок
// let lastPanel = panels[panels.length - 1];

// // обновляем после загрузки шрифтов / картинок
...
//   trigger: ".standart__container",
//   start: "top top",
//   endTrigger: lastPanel,
//   end: `top top+=${(panels.length - 1) * offsetGap}`,
//   pin: titlee,
//   pinSpacing: false,
// });
...
//   // PIN панели ниже заголовка
//   ScrollTrigger.create({
//     trigger: panel,
//     start: `top top+=${titleOffset + i * offsetGap}`,
//     endTrigger: lastPanel,
//     end: `top top+=${titleOffset + (panels.length - 1) * offsetGap}`,
//     pin: true,
//     pinSpacing: false,
//     scrub: true
//   });

//   // SCALE предыдущих панелей
//   gsap.to(panel, {
//     scale: 1 - (panels.length - i) * 0.015,
//     ease: "none",
//     scrollTrigger: {
//       trigger: panel,
//       start: `top top+=${titleOffset + i * offsetGap}`,
...
let standartTitle = document.querySelector('.standart__title')
if(standartTitle){
	standartTitle = standartTitle.offsetHeight
let standartItems = document.querySelectorAll('.standart__item');

for (let index = 0; index < standartItems.length; index++) {
	const element = standartItems[index];
	element.style.top = standartTitle + (index * 20) + 'px'
}
}
...
window.addEventListener('scroll', () => {
  panels.forEach((panel, index) => {
    ...
    // момент, когда следующая начинает заходить
    const overlap = panelRect.bottom - nextRect.top;
console.log(overlap)
    if (overlap > 0) {
      // высота панели для нормализации
          const progress = Math.min(overlap / (panelRect.height * 3), 1);
      ...
      // возвращаем в норму при скролле вверх
...
document.querySelectorAll('.calculation__wp').forEach(wrapper => {
  let minus = wrapper.querySelector('._minus');
  let plus = wrapper.querySelector('._plus');
  let valueEl = wrapper.querySelector('.calculation__value');

  let STEP = 10000;
  let MIN = 0;

  function parseValue() {
    return parseInt(valueEl.textContent.replace(/\D/g, ''), 10) || 0;
  }
...
// создаём скрытый span для измерения текста
...
// копируем стили текста из input
...
// форматирование + ресайз при вводе
...
// let wrapper = document.querySelector('.wrapperr');
// let frameRate = 2;
// let minScale = 0.3;
// let maxScale = 1.1;
...
//     let childX = r.right - (r.width / 2);
//     let childY = r.bottom - (r.height / 2);
//     let distanceY = Math.max(origin.y, childY) - Math.min(origin.y, childY);
//     let distanceX = Math.max(origin.x, childX) - Math.min(origin.x, childX);
...
//     let relativeAmt = (index / children.length) * scaleRange;
//     child.style.setProperty('--scale', minScale + relativeAmt);
//   });
// }

// // общий update для десктопа и мобилки
// function updateEffect(x, y) {
//   let screenCenter = {
//     x: window.innerWidth / 2,
//     y: window.innerHeight / 2
//   }
//   let moveX = x - screenCenter.x;
//   let moveY = y - screenCenter.y;

//   wrapper.style.setProperty('--x', moveX / 10 + '%');
//   wrapper.style.setProperty('--y', moveY / 10 + '%');

//   animateChildren(wrapper, {x, y});
// }

// // ===== DESKTOP =====
// document.addEventListener('mousemove', function (e) {
//   requestAnimationFrame(function (thisFrame) {
//     if (!lastFrame || thisFrame - lastFrame > frameRate) {
...
//   let t = e.touches[0];
//   updateEffect(t.clientX, t.clientY);
// }, {passive: true});
...
let centerX = scene.offsetWidth / 2;
let centerY = scene.offsetHeight / 2;

let swiperBanksSlides = document.querySelectorAll('.banks__slide');

let icons = [...swiperBanksSlides].map(slide => {
...
function createIcons() {
  ...
  for (let i = 0; i < VERTICAL; i++) {
    for (let j = 0; j < HORIZONTAL; j++) {
      let iconObj = icons[iconIndex % icons.length]; 
      iconIndex++;
// создаём div
...
container.appendChild(el);
  ...
  let gridWidth = maxX - minX + SIZE;
  let gridHeight = maxY - minY + SIZE;
...
function clampOffsets() {
  ...
  // учтём размеры иконок
  let leftMost = minX - SIZE / 2;
  let rightMost = maxX + SIZE / 2;
  let topMost = minY - SIZE / 2;
  let bottomMost = maxY + SIZE / 2;

  // пределы смещения по краям экрана
  let minOffsetX = Math.min(0, window.innerWidth - rightMost);
  let maxOffsetX = Math.max(0, -leftMost);

  let minOffsetY = Math.min(0, window.innerHeight - bottomMost);
  let maxOffsetY = Math.max(0, -topMost);
...
function animatee() {
  for (let circle of circles) {
    let dx = circle.x - centerX + offsetX;
    let dy = circle.y - centerY + offsetY;
    let dist = Math.sqrt(dx * dx + dy * dy);

    let scale = Math.max(0.45, 1 - dist / SCALE_FACTOR);
...
container.addEventListener('click', e => {
  let el = e.target.closest('.icon');
  if (!el) return;

  let circle = circles.find(c => c.el === el);
  if (!circle) return;

  // Проверяем, в центре ли элемент
  let screenCenterX = centerX;
  let screenCenterY = centerY;

  let elScreenX = circle.x + offsetX;
  let elScreenY = circle.y + offsetY;

  let tolerance = 5; // px, погрешность
  ...
  let targetOffsetX = centerX - circle.x;
  let targetOffsetY = centerY - circle.y;
...
// отключаем скролл только когда палец внутри контейнера
...
scene.addEventListener('touchmove', e => {
    e.preventDefault(); // блокируем прокрутку body
    offsetX = oldOffsetX + e.touches[0].clientX - startX;
    offsetY = oldOffsetY + e.touches[0].clientY - startY;
    clampOffsets();
...
document.addEventListener('click', e => {
  let btn = e.target.closest('._pay-video');
  if (!btn) return;
  ...
  let preview = slide.querySelector('.speack__prev');
...
document.addEventListener("click", function(e) {
  ...
  let current  = dropdown.querySelector(".dropdown__current");
  ...
  // Клик по пункту
  let item = e.target.closest(".dropdown__item");
  if (item) {
...
document.querySelectorAll('.dropdown').forEach(dropdown => {
  let selected = dropdown.querySelector('.dropdown__item[data-selected]');
  let current = dropdown.querySelector('.dropdown__current');
...
let creditsDropBtn = document.querySelector('.credits__drop-btn');
let creditsDropdowns = document.querySelector('.credits__dropdowns');
let creditsDropdownsBtn = document.querySelector('.credits__dropdowns-top button');
if(creditsDropBtn){
	creditsDropBtn.addEventListener('click', function(){
creditsDropdowns.classList.add('_active')
	})
	creditsDropdownsBtn.addEventListener('click', function(){
creditsDropdowns.classList.remove('_active')
	})
...
document.addEventListener('click', (e) => {
  let menu = document.querySelector('.header__s');

  if (!menu) return;
console.log(e.target)
  // если клик НЕ внутри меню
  if (!menu.contains(e.target)) {
    document.querySelector('.header__search-menu').classList.remove('_active');
  }
});
...
document.querySelector('.header__search-menu').classList.add('_active')
	})
...
headerInput.addEventListener('input', function(){
	if(headerInput.value.length > 0){
		document.querySelector('.header__search-menu').classList.add('_open')
	}else{
		document.querySelector('.header__search-menu').classList.remove('_open')
	}
})
...
if(headerMenuClose){
	headerMenuClose.addEventListener('click', function(){
		document.querySelector('.header__links').classList.remove('_active')
		document.querySelector('.header__burger').classList.remove('_active')
	})
}


let speackVideo = document.querySelectorAll('.speack__video');

for (let index = 0; index < speackVideo.length; index++) {
	let element = speackVideo[index];
	element.addEventListener('click', function(){

		let videoId = element.dataset.videoId;

  document.querySelector('.popup__video').innerHTML = `
    <iframe
  ...
  `;
		document.querySelector('.popup_video').classList.add('_active')
	})
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  parallaxSections.forEach(section => {
    const items = section.querySelectorAll('.parallax');
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        if (entry.isIntersecting && !section.hasInitialSet) {
          items.forEach(item => {
            item.initialTop =
              item.getBoundingClientRect().top + window.scrollY;
          });
          section.hasInitialSet = true;

          // сразу пересчитываем позицию
        //   onScroll();
        }
      });
    }, { threshold: 0 });
    ...
    const onScroll = () => {
      if (!section.active || !section.hasInitialSet) return;

      const scrollY = window.scrollY;

      items.forEach(item => {
        const speed = Number(item.dataset.speed) || 0.3;

        const delta = scrollY - item.initialTop;
        const y = delta * speed;
    ...
    // если страница загружена уже с прокруткой
    onScroll();
  });
});
...
let articlesOpenFilter = document.querySelector('.articles__open-filter');
let articlesMob = document.querySelector('.articles-mob');
let articlesClises = document.querySelectorAll('.articles-close');

if(articlesOpenFilter){
	articlesOpenFilter.addEventListener('click', function(){
		articlesMob.classList.add('_active')
	})
}

for (let index = 0; index < articlesClises.length; index++) {
	const element = articlesClises[index];
	element.addEventListener('click', function(){
		articlesMob.classList.remove('_active')
	})
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const observer = new IntersectionObserver((entries) => {
    ...
    threshold: 0.6 // когда 60% блока видно
  });
...
const slides = document.querySelectorAll('.common__slide');

slides.forEach((slide, index) => {
  const i = index + 1; // делаем как nth-child (с 1)
  ...
  if (i % 4 === 2) {
    ...
    order = i; // остальные без изменений
  }
